...
describe("Unit testing of project contract", async function () {
  ...
  it("it should update the projectdata ", async () => {
    ...
    var txn = await tx.wait();
    var tx1 = await project.getProjectTimestamps(projectId);


    var result1 = await project.getProjectData(projectId, tx1[0]);
    var result2 = await project.getProjectData(projectId, tx1[1]);

    expect(tx1[0]).to.equal(result1.updatedAt)
    expect(tx1[1]).to.equal(result2.updatedAt)


    // var Id2 = await project.getProjectData(projectId, tx1[1]);
    // console.log(`timestamps ========> ${tx1[1]}  1 ======>,${Id2}`);
  });
});
